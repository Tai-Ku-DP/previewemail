...
export default function TemplateLibraryPage() {
  ...
  const handleUseTemplate = useCallback(
    async (pt: PredefinedTemplate) => {
      try {
        let layoutId = undefined;
        if (pt.layoutHtmlBody) {
          const layout = await createLayout(
            `${pt.name} Layout`,
            `imported-layout-${Date.now()}`,
          );
          await updateLayout(layout.id, {
            htmlBody: pt.layoutHtmlBody,
            textBody: "",
          });
          layoutId = layout.id;
        }
  ...
  return (
    <div className="flex h-screen flex-col bg-bg">
      ...
      <main className="mx-auto flex w-full max-w-5xl min-h-0 flex-1 flex-col px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-10 pr-2">
          {filteredTemplates.map((t) => (
            <div
              key={t.id}
              className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-bg transition-all hover:shadow-md hover:border-fg-muted/30"
            >
                  ...
                  <iframe
                    srcDoc={wrapIframeSrcDoc(
                      t.layoutHtmlBody
                        ...
                        : (compileTemplate(t.htmlBody, t.mockData || {})
                            .result ?? ""),
                    )}
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <p className="mt-1.5 flex-1 text-[13px] text-fg-muted line-clamp-2 leading-relaxed">
                  {t.description}
                </p>
      ...
      {previewTemplate && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 sm:p-6 md:p-12 animate-in fade-in duration-200"
          ...
          >
            <div className="flex h-12 shrink-0 items-center justify-between border-b border-border px-4 bg-bg-subtle">
              <div className="flex items-center gap-3">
                ...
                <span className="rounded-full bg-border px-2 py-0.5 text-[10px] font-medium text-fg-muted uppercase tracking-wider">
                  Preview
                </span>
              </div>
              <div className="flex items-center gap-2">
